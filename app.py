...
def main() -> None:
    ...
    env_authorized = os.getenv("AUTHORIZED", "false").lower() == "true"
    authorized = args.authorized or env_authorized
    if not authorized:
        logging.error("Authorization required. Use --authorized flag or set AUTHORIZED=true.")
        raise SystemExit(1)
    ...
    try:
        result = run_crew(domain, authorized=authorized)
